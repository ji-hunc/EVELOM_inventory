...
export default function InventoryTable({
  ...
  const { alertEnabled } = useAuth();
  const [selectedCategory, setSelectedCategory] = useState<string>("all");
  const [sortBy, setSortBy] = useState<
    "name" | "category" | "stock" | "batch_code"
  >("name");
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("asc");
  // 카테고리별 정렬 상태 관리
  const [categorySortStates, setCategorySortStates] = useState<
    Record<
      string,
      {
  ...
  const [tableViewMode, setTableViewMode] = useState<
    "single" | "category-separated"
  >("category-separated");

  // 위치가 변경될 때 아코디언 상태 초기화
  useEffect(() => {
  ...
  const filteredInventory = inventory.filter((item) => {
    if (tableViewMode === "category-separated" || selectedCategory === "all")
      return true;
    ...
    tableViewMode === "category-separated"
      ? categoryOrder.reduce(
          (acc, categoryName) => {
            ...
            const categoryInventory = filteredInventory.filter(
              (item) => item.product?.category_id === category.name
            );
  ...
  const handleCategorySort = (
    categoryName: string,
    field: "name" | "category" | "stock" | "batch_code"
  ) => {
    const currentState = categorySortStates[categoryName] || {
      sortBy: "name",
      sortOrder: "asc",
    };
  ...
  // 카테고리별 정렬된 인벤토리 반환
  const getSortedCategoryInventory = (
    categoryName: string,
    groupedInventory: GroupedInventoryItem[]
  ) => {
    const sortState = categorySortStates[categoryName] || {
      sortBy: "name",
      sortOrder: "asc",
    };
  ...
  const isLowStock = (stock: number) => alertEnabled && stock <= alertThreshold;
    ...
    setExpandedGroups((prev) => {
      ...
      if (newSet.has(groupKey)) {
        ...
        if (tableViewMode === "single") {
          totalExpandableGroups = sortedGroupedInventory.filter(
            (group) => group.batch_count > 1
          ).length;
        } else if (inventoryByCategory) {
          Object.values(inventoryByCategory).forEach((catData) => {
            totalExpandableGroups += catData.groupedInventory.filter(
              (group) => group.batch_count > 1
            ).length;
          });
        }
  ...
  const hasExpandableGroups =
    tableViewMode === "single"
      ...
      : inventoryByCategory &&
        Object.values(inventoryByCategory).some((catData) =>
  ...
  const handleEditMode = () => {
    if (isEditMode) {
      ...
      // 일괄수정 모드 진입시 "모두펼치기" 버튼과 동일한 효과 적용
      if (!allExpanded) {
        let allGroupKeys: string[] = [];
  ...
  const handleDeleteProduct = async (item: Inventory) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/products/${item.product_id}?${params.toString()}`,
        {
          method: "DELETE",
        }
      );
  ...
  const renderCategoryTable = (
    ...
    const { category, groupedInventory: categoryGroupedInventory } =
      categoryData;
    // 카테고리별 독립적인 정렬 적용
    const sortedCategoryInventory = getSortedCategoryInventory(
      categoryName,
      categoryGroupedInventory
    );
    const categorySort = categorySortStates[categoryName] || {
      sortBy: "name",
      sortOrder: "asc",
    };
        ...
        <div className="overflow-x-auto">
          <table
            className={`min-w-full divide-y divide-gray-200 ${
              showImagesLocal ? "table-auto" : "table-fixed"
            }`}
            style={showImagesLocal ? { width: "max-content" } : undefined}
          >
                ...
                {showImagesLocal && (
                  <th
                    className={`px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${
                      showImagesLocal ? "" : "w-20"
                    }`}
                    style={showImagesLocal ? { width: "100px" } : undefined}
                  >
                    이미지
                  </th>
                )}
                        ...
                        <td className="px-4 py-3">
                          {isEditMode && !hasMultipleBatches ? (
                            <input
                              type="number"
                              value={
                                editValues[firstBatch.id] ??
                                firstBatch.current_stock
                              }
                              onChange={(e) =>
                                handleStockChange(firstBatch.id, e.target.value)
                              }
                        ...
                        <td className="px-4 py-3 text-sm text-gray-500">
                          {hasMultipleBatches ? (
                            ...
                            <div className="flex flex-col">
                              <span>
                                {formatKoreanDate(firstBatch.expiry_date)}
                              </span>
                              {(() => {
                        ...
                        <td className="px-4 py-3 text-right">
                          {hasMultipleBatches ? (
                            ...
                            <div className="flex items-center justify-end gap-2">
                              {!isAllSelected &&
                                canEditLocation(group.location_id) && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
